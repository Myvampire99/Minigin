...
ThreadEnemyLevel::ThreadEnemyLevel(std::string name)
	:Scene(name)
	, m_IntHealth{ 3 }
{
...
ThreadEnemyLevel::~ThreadEnemyLevel()
{
	StopThreading();
...
void ThreadEnemyLevel::SwitchSceneIni() {
...
void ThreadEnemyLevel::SwitchSceneDec() {
...
void ThreadEnemyLevel::SceneInitialize() {
...
void ThreadEnemyLevel::CreateWall(int start, int max, int inc) {
...
void ThreadEnemyLevel::CreatePlayer() {
	...
	//Input
	auto m_Inputcomponent = new InputComponent(0);
	...
	m_Player->SetPosition((float)margin * 2, (float)margin * 2);

	//
	m_GridManager->GetClosestNode({ (float)margin * 2,  (float)margin * 2 })->object->MarkForDelete();
	m_GridManager->FreeBlock(m_GridManager->GetClosestNode({ (float)margin * 2, (float)margin * 2 }), std::make_shared<dae::GameObject>());
	//
...
void ThreadEnemyLevel::CreateSnoBee(dae::Vector2 pos, int p) {
...
void ThreadEnemyLevel::CreateAnIceBlockEgg(int IDs) {
...
void ThreadEnemyLevel::CreateAnIceBlock(int IDs) {
...
void ThreadEnemyLevel::SceneUpdate() {
...
void ThreadEnemyLevel::LocalReset() {
